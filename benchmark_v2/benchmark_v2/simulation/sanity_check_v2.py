    ...
    for it in types:
        for seed in range(30):
            ...
            for s in meta["sources"]:
                ...
                if abs(f) + bw / 2.0 > 0.5 + 1e-9:
                    bad += 1
    check("fold: 全部类型频带包含于奈奎斯特带", bad == 0, f"violations={bad}")
...
def test_bandwidth():
    ...
    # 端到端：元数据带宽 == actual_bandwidth
    for seed in range(5):
        _, meta = generate_signal_sample(num_sources=3, seed=seed)
        tgt = meta["sources"][0]
        expect_t = actual_bandwidth(tgt["modulation"], 1024)
        check(f"bandwidth: 目标元数据=实际 (seed={seed})",
              abs(tgt["bandwidth_normalized"] - expect_t) < 1e-9,
              f"{tgt['bandwidth_normalized']:.5f} vs {expect_t:.5f}")
        for s in meta["sources"][1:]:
            bw = s["bandwidth_normalized"]
...
def test_dataset(count=10):
    ...
    try:
        ...
        # 目标元数据带宽 == 实际生成带宽（意见1 修复验证）
        bw_ok = all(
            abs(s["metadata"]["sources"][0]["bandwidth_normalized"]
