...
def spectrogram_image(iq, sampling_rate_hz=20e6, nperseg=128, noverlap=None,
    ...
    # mode="psd" 返回的已是功率谱密度（实数组），直接取绝对值，不得再平方
    psd = np.abs(S)
...
def music_spectrum(iq, num_sources=None, doa_range=(-90.0, 90.0), num_points=361,
    ...
    # 分母 = ||En^H·a(θ)||² = a^H·En·En^H·a。注意必须是 a^H 在前、a 在后：
    # 写成 a·En·En^H·a^H 对复数 steering vector 是共轭形式，谱峰符号会反转。
    proj = np.real(np.einsum("pm,mn,pn->p", a.conj(), enen, a))
