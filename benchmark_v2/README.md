...
| **V2（当前）** | 4 阵元 ULA、多源（1 目标 + 0~2 干扰）、复杂干扰信号体制（真实辐射体简化建模：WiFi/LTE/蓝牙/雷达/GPS + 脉冲串/NFM 等）、物理保真（LNA 饱和 / 阵元失配 / 防折叠 / 带宽统一），完整评估链路（检测 / 分类 / 参数估计 / DOA） | ✅ 完成（v8 候选位置切片 + 混合对齐体制识别，500 样本基线 ds_run19） |
...
└── benchmark_v2/                     # V2（当前，本目录）
    ...
    ├── mod_classifier/               # 学习型体制分类器（训练/验收管线 + 三级 oracle）
    └── ds_run19.json                 # 500 样本最终基线结果（v8）
```
...
| `estimate_modulation_features` | **信号体制识别**（答案空间 10 类 = 5 种辐射体调制 BPSK/QPSK/GFSK/LFM/OFDM + 模拟调频 NFM + 4 种简单波形）：学习型分类器后端（1D-CNN，feature_distance = 1 − softmax 概率，top-3）；**v8 切片按空间候选位置优先**（覆盖率 0.67→0.935，含同频）+ 混合对齐训练（切片含目标泄漏，与部署同分布）；三级 oracle：clean 0.870 / GT 切片 0.83+ / 工具视角 0.656。`use_cnn=False` 回退手工特征模板法。调制为辐射体溯源的简化代理（见第 4 节标签轴说明） |
...
| 第 7 版全量基线（500 样本，学习型体制分类器，ds_run18） | deepseek-v3.2 | 0.714 | 0.752 | 0.836 | 0.740 | 0.876 | 0.45 / 0.341 | 4h02m |
| **第 8 版全量最终基线（500 样本，候选位置切片 + 混合对齐重训，ds_run19）** | deepseek-v3.2 | 0.702 | **0.758** | **0.848** | **0.758** | **0.881** | 0.45 / **0.554** | 4h00m |
...
> **第 7 版（ds_run18）**：体制识别后端升级——`estimate_modulation_features` 换用**学习型分类器**（`mod_classifier/`，1D-CNN 0.15M 参数；训练集独立配置生成、种子段/带宽/SNR 范围与考卷隔离，冻结 500 不进训练；两级 oracle 预注册验收 clean **0.873** / 切片 **0.688** 过闸），feature_distance = 1 − softmax 概率，Agent 契约不变，`use_cnn=False` 回退旧后端。旧"手工特征+模板匹配"范式经 oracle 实测干净上限仅 0.66（物理封顶）。端到端：调制 e2e 0.201→**0.341**（1.7×）、matched 0.265→**0.452**，其余五项指标零回退。
> **第 8 版（当前，ds_run19）**：两项对齐修复——①切片改造：`estimate_modulation_features` 优先按空间候选位置切片（v6 候选层的精确 freq/bw 一直没被调制工具利用；同频干扰不再被跳过），切片覆盖 0.67→**0.935**，上限 6 片，合并组补充；②混合对齐重训：训练分布加入 40% 目标泄漏混合切片（目标 0dB + 干扰功率比 U(-5,15)dB），训练视角 = 部署视角；配套 FM 家族扩样。三级 oracle（新增工具视角门槛 ≥0.65）：clean 0.870 / GT 切片调制类 **0.891** / 工具视角 0.424→**0.656**。端到端：调制 e2e 0.341→**0.554**（1.6×）、matched 0.452→**0.711**，其余六项持平或微升（频偏/带宽/DOA 达 v2 历史最佳）。
> 已知代价与限制（v8 复核）：①体制识别端到端 0.554 vs 工具视角 0.656——残余差距来自部分同角双干扰样本的空间候选缺失（生成约束允许 Δfreq 替代 ΔDOA，4 阵元对 <3° 角间隔不可分辨，物理限制）；②LFM↔swept 标签重叠（swept 默认即脉冲化 LFM，同族信号）——类别设计问题，待确认合并或区分；③GFSK↔nfm 调频家族残余混淆；④源数 0.702（v7 0.714，波动范围内）。

> 工具层参考精度：频偏估计 0.94（±0.02 容差）、DOA 1.00（±12°，INR≥8）、源数（MDL）0.80——即**工具测量本身很准，Agent 综合判断是主要损耗点**。

### 10.2 关键指标细节（最终基线 ds_run19，500 样本）

- **参数估计数值精度**：频偏 MAE 0.008（容差 ±0.02，命中 0.848）、DOA MAE 0.76°（容差 ±10°，命中 0.881）、带宽命中 0.758；
- **源数（源级匹配统计）**：漏检 99 / 误报 163（v5：231/297）；
- **类别按 SNR**：Low 0.644 / Mid 0.808 / High 0.768（n=104/200/196）；
- **调制识别（体制）**：e2e 0.554 / matched 0.711——v5 起点 0.166，累计 **3.3×**；仪器口径（工具切片三级验收）0.656；训练/验收协议与上限实验见 `mod_classifier/`。

### 10.3 结论

1. **三次架构级仪器修复构成 v2 的主收益**：空间候选层（类别 0.588→0.754、DOA MAE 3.63°→0.76°）、学习型体制分类器 + 候选位置切片/混合对齐（调制 e2e 0.166→**0.554**）——全部按"预注册门槛 + 零 token 离线验收 → 50 样本在线确认 → 500 全量基线"流程完成，八项指标无回退；
2. **调制识别的 0.5 目标达成**：损失分解诊断（切片覆盖 58% 是主因）→ 候选位置切片（覆盖 93.5%）→ 混合对齐重训（工具视角 0.424→0.656）——"训练视角 = 部署视角"两次成为胜负手；
3. **剩余瓶颈与 V3 衔接**：低 INR 弱源覆盖（物理）、同角双干扰空间不可分辨（物理）、LFM↔swept 类别重叠（设计问题待确认）；`benchmark_v3/` 低空环境信号模型草案已就绪（辐射体池/低空信道/溯源标签轴），待辐射体清单与任务粒度确认后定稿。仿真侧 v2 至此收口。
