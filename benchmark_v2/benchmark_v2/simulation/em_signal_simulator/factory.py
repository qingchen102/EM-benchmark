...
from .baseband import generate_baseband, default_bandwidth, actual_bandwidth, MODULATIONS
...
def _sample_interferer(rng, num_samples, force_type=None, **kw):
    ...
    if typ in JAMMING_WAVEFORMS:
        ...
        if typ == "swept":
            ...
            # 防折叠：f_dev + noise_bw <= 0.45，保证 NFM 频带 ±(f_dev+noise_bw) ⊂ Nyquist
            if wave_kw["freq_deviation"] + wave_kw["noise_bandwidth"] > 0.45:
                scale = 0.45 / (wave_kw["freq_deviation"] + wave_kw["noise_bandwidth"])
        ...
        if typ == "single_tone":
            ...
            bandwidth = min(
                1.0,
                2.0 * (float(wave_kw.get("freq_deviation", 0.2))
        ...
        offset_lo, offset_hi = _valid_offset_range(bandwidth)
    ...
    return {
        ...
        "doa_degree": float(rng.uniform(*DOA_RANGE)),
        "power_ratio_db": power_ratio_db,
        "bandwidth_normalized": bandwidth,   # 实际带宽（生成与元数据一致）
        "is_pulse": False,
        "is_modulated": True,
        "wave_kw": {},
    }
    ...
    for _ in range(int(max_attempts)):
        ...
        for i in range(len(interferers)):
            for j in range(i + 1, len(interferers)):
                ...
                b_bounds = _valid_offset_range(b.get("bandwidth_normalized", 0.0))
    ...
    for i in range(len(interferers)):
        for j in range(i + 1, len(interferers)):
            ...
            if df < freq_min and ddoa < doa_min:
                ...
                lo_b, hi_b = _valid_offset_range(b.get("bandwidth_normalized", 0.0))
...
def generate_signal_sample(
    ...
    **kwargs,
    ...
    sources_meta = [{
        ...
        "doa_degree": 0.0,
        "bandwidth_normalized": target_bw,
        "bandwidth_hz": _normalized_to_hz(target_bw, fs),
    }]
    ...
    if interferers:
        ...
        for i, itf in enumerate(interferers):
            ...
            sources_meta.append({
                ...
                "bandwidth_normalized": bw_actual,
                "bandwidth_hz": _normalized_to_hz(bw_actual, fs),
