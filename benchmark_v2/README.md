...
| **第 6 版全量最终基线（500 样本，MVDR 频率×角度候选层，ds_run17）** | deepseek-v3.2 | **0.696** | **0.754** | **0.836** | **0.745** | **0.868** | 0.27 / **0.201** | 4h50m |
...
> **第 6 版（当前，ds_run17）**：候选层升级——`analyze_spectrum` 默认改用 **MVDR 频率×角度联合分析**（`spatial_candidates.py`）：分段 STFT 估计每频点空间协方差 + Capon 波束扫描，功率谱上熔住的源按方位角分辨（利用生成约束 Δfreq≥0.15 或 ΔDOA≥15° 保证的角度可分性；旧一维路径把该维度在功率合成一步丢掉，是 v5 候选覆盖 57% 的根因），候选新增 `angle_deg`，功率在源自身波束内测量（目标被零陷，4.5dB 标定常数不再需要）。500 样本离线 A/B（`diag_spatial_ab.py`）：候选覆盖 57%→**87%**（宽带 39%→**72%**）、功率 MAE 4.8→**1.8dB**、干净样本幽灵 49→**8**。端到端：类别 +0.166、DOA MAE ÷3.8、漏检 231→**107**。
> 已知代价与限制（v6 复核）：①blocking 假阳 67→**14** 个/500（±4dB 硬币区被 ±1.8dB 测量精度瓦解，残余为贴门限样本），压缩特征仍仅作边界带辅助证据；②宽带覆盖 0.72、低 INR（≤6dB）邻道覆盖 0.75，弱源仍受物理限制；③源数仅微升（0.68→0.70）——`estimate_num_sources` 尚未接入空间证据（MDL + 角度脊联合为下一步）；④调制 e2e 0.201，7 维特征瓶颈不变（C40/C42 战线独立推进，上限 0.266）。

> 工具层参考精度：频偏估计 0.94（±0.02 容差）、DOA 1.00（±12°，INR≥8）、源数（MDL）0.80——即**工具测量本身很准，Agent 综合判断是主要损耗点**。

### 10.2 关键指标细节（最终基线 ds_run17，500 样本）

- **参数估计数值精度**：频偏 MAE 0.009（容差 ±0.02，命中 0.836）、DOA MAE 0.96°（容差 ±10°，命中 0.868）、带宽命中 0.745；
- **源数（源级匹配统计）**：漏检 107 / 误报 153（v5：231/297）；平均工具调用 5.09/样本，工具调用率 100%；
- **类别按 SNR**：Low 0.678 / Mid 0.785 / High 0.763（n=104/200/196）——v5 的"高 SNR 不高于 Mid"系统性异常基本消除；
- **调制识别**：e2e 0.201 vs 同特征上限 0.266（`eval_mod_upperbound.py` 离线验证）——7 维特征下 QPSK/GFSK ~4%、LFM ~14%，升级方向为 C40/C42 高阶累积量 + 包络统计。

### 10.3 结论

1. **空间维度是 v2 最大的单笔收益**：候选层接入 MVDR 频率×角度后，类别 0.588→**0.754**、DOA MAE 3.63°→**0.96°**、漏检 231→**107**——验证了 v5 诊断出的分层归因："瓶颈在测量层的信息利用，不在模型"；
2. **blocking 从结构性无解到近满召回**：功率在源自身波束内测量（目标被零陷），假阳 67→14、召回 93%（匹配对 56→92）——v5 的"±4dB 测量精度 vs 10dB 门限"固有权衡被瓦解；
3. **剩余瓶颈与 V3 衔接**：源数（MDL 未用空间证据，下一步 MDL+角度脊联合）、调制特征（C40/C42，离线管线就绪）、低 INR 弱源覆盖（物理限制）；仿真侧收益再次接近榨干，真实 IQ 采集（P0）可并行启动——本版 MVDR 候选层即 v3 真实阵列数据的同一套代码路径。
