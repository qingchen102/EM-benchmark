...
└── simulation/                        # 模块根目录
    ...
    └── em_signal_simulator/           # 核心算法库
        ...
        ├── baseband.py                # 目标基带波形生成（9 种调制 + 带宽可配 + 真升余弦）
...
```json
{
  ...
  "metadata": {
    ...
    "lna_saturation": {
      "applied": true,
      "model": "rapp",
      "a_sat": 1.9475967981153213,
      "p": 2.0,
      "saturation_db": 6.0
    },
    "array_mismatch": {
      "gain_error_std": 0.05,
      "phase_error_std_deg": 5.0
    },
    "sources": [
      {
        "source_id": 0,
        ...
        "freq_offset_hz": 0.0,
        "doa_degree": 0.0,
        "bandwidth_normalized": 0.16875,
        "bandwidth_hz": 3375000.0
      },
      {
        "source_id": 1,
        "role": "interferer",
        "modulation": "OFDM",
        "display_name": "WiFi_OFDM",
        "waveform_type": null,
        "power_ratio_db": 5.713235279821088,
        "inr_db": 15.713235279821088,
        "freq_offset_normalized": 0.37863311494823637,
        "freq_offset_hz": 7572662.298964728,
        "doa_degree": 38.14798765208411,
        "bandwidth_normalized": 0.15625,
        "bandwidth_hz": 3125000.0,
        "v2_category": "none",
        "is_pulse": false,
        "is_modulated": true,
        "waveform_params": {}
      },
      {
        ...
        "display_name": "WiFi_OFDM",
        "waveform_type": null,
        "power_ratio_db": 12.315484105511166,
        "inr_db": 22.315484105511167,
        "freq_offset_normalized": 0.2161699773739902,
        "freq_offset_hz": 4323399.547479805,
        "doa_degree": 1.8599167280525535,
        "bandwidth_normalized": 0.40625,
        "bandwidth_hz": 8125000.0,
...
- `bandwidth_normalized`：**实际占用带宽**（由生成参数计算，与信号频谱一致）—— 目标 = `actual_bandwidth(modulation)`，真实调制干扰 = 随机采样的实际带宽，纯波形干扰按语义计算（单音 0 / 扫频范围 / NFM Carson 近似等）
...
```bash
# 全量验证：5 大任务功能 + 端到端（形状/dtype/NaN/元数据/隔离/可视化/并行）
