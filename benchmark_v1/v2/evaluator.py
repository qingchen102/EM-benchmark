...
Diagnostic Rules:
1. 'none': Occupied bandwidth is small/normal, PAPR is standard (3-6 dB), no extreme high-power peaks or abnormal pulse duty cycle.
2. 'single_tone': Extremely high peak magnitude in spectrum at a specific normalized peak frequency, narrow bandwidth (< 0.05).
3. 'broadband': High average magnitude across all frequencies, large occupied bandwidth (> 0.5), low/normal PAPR, spectral flatness > 0.6.
4. 'pulse': High PAPR (> 8 dB) and high power pulse threshold with duty cycle < 0.35.
5. 'swept': Moderate-to-high occupied bandwidth with dynamic spectral spreading and high peak frequency drift std over time.
...
class OpenAICompatibleAgent:
    ...
    def diagnose(self, sample_path: Path) -> tuple[dict[str, Any], int, int]:
        messages: list[dict[str, Any]] = [
            ...
            {"role": "user", "content": f"Analyze sample at {sample_path}. Use both tools before diagnosing."}
        ]
        calls = successes = 0
        for _ in range(5):
...
def evaluate_dataset(dataset_dir: str | Path, agent: OpenAICompatibleAgent | None = None) -> dict[str, Any]:
    ...
    for record in pbar:
        ...
        tqdm.write(f"[{sample.name}] GroundTruth: {expected:<12} | Predicted: {predicted:<12} {status_icon}")
