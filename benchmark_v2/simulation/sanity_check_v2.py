...
def test_observation_gt_split():
    ...
    try:
        ...
        check("split: observation 含接收机/目标调制/目标带宽",
              "receiver" in o0 and "target_modulation" in o0
              and "target_bandwidth_normalized" in o0
...
def test_evaluator_v2():
    ...
    try:
        sp = vdir / "s.npy"
        np.save(sp, iq)
        # 工具冒烟（analyze_spectrum 传入目标带宽，验证 ratio 计算）
        tgt_bw = meta["sources"][0]["bandwidth_normalized"]
        outs = [
            analyze_spectrum(str(sp), target_bandwidth_normalized=tgt_bw),
        ...
        check("ev2: 频谱含峰/带宽/候选源", "peaks_normalized" in outs[0]
              and "obw99_normalized" in outs[0] and "sources_candidates" in outs[0])
        check("ev2: 候选源含 ratio", len(outs[0]["sources_candidates"]) > 0
              and "ratio_to_target_bw" in outs[0]["sources_candidates"][0])
        check("ev2: 源数默认建议在范围", 1 <= outs[1]["num_sources_estimate"] <= 3)
        check("ev2: 源数默认建议 = MDL", outs[1]["mdl_estimate"] is None
              or outs[1]["num_sources_estimate"] == outs[1]["mdl_estimate"])
        check("ev2: 源数证据三件套", all(k in outs[1] for k in
              ("mdl_estimate", "spectral_peak_count", "merged_peak_count",
               "consistent", "confidence")))
        check("ev2: 源数证据取值一致",
              (outs[1]["mdl_estimate"] is None) or (
                  1 <= outs[1]["mdl_estimate"] <= 3
                  and 1 <= outs[1]["spectral_peak_count"] <= 3
                  and 1 <= outs[1]["merged_peak_count"] <= 3))
        check("ev2: DOA 跨阶稳定峰/高阶新峰字段",
              isinstance(outs[2].get("stable_peaks_deg"), list)
              and isinstance(outs[2].get("peaks_new_at_order3_deg"), list))
