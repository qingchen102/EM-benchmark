...
- freq_offset   <- analyze_spectrum peaks_excluding_center (interferer candidates,
                  target sits near 0 Hz), else peaks_normalized
- bandwidth     <- analyze_spectrum per_peak_bandwidth for the peak matching the
                  interferer (fall back to obw99_normalized if peaks overlap)
- doa           <- estimate_doa doa_estimates_deg
- num_interferers <- estimate_num_sources num_sources_estimate MINUS the target (1 source is the target)
- modulation    <- estimate_modulation_features features + per_peak template_distances
                  (compare distances across candidates; smaller distance = closer match)

Interference category definition (ratio = |freq_offset| / target bandwidth):
- pulse:      pulsed waveforms -> high PAPR, low duty cycle, periodic bursts
- co_channel: ratio < 0.5 (interferer inside the target band)
- blocking:   ratio >= 0.5 AND interferer power >= 10 dB above target
              (strong out-of-band signal -> receiver LNA saturation, distorts target)
- adjacent:   0.5 <= ratio < 2.0 with weaker power
- none:       ratio >= 2.0 with weaker power (harmless distant weak signal)
